...
| `backend/media/` | همان چیزی که nginx روی `/media/` سرو می‌کند — **۵۳ فایل** | هرگز |
| پوشهٔ هاست `backend/media` (بایند-مانت به `/app/media` در `docker-compose.yml`) | **جایی که اپ آپلودها را می‌نویسد — ۴۷ فایل** | هرگز |
...
> پس بخشی از آپلودها (۴۷ در برابر ۸ فایل) روی سایت ۴۰۴ می‌دهد. یکسان‌کردن این دو اکنون با بایند-مانت در `docker-compose.yml` حل شده است.
>
