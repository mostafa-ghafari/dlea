...
"%WHEEL_ENV%\Scripts\pip.exe" download -r backend\requirements.txt -d pip-wheels --platform manylinux2014_x86_64 --platform manylinux_2_17_x86_64 --platform any --python-version 312 --only-binary=:all:

if %errorlevel% neq 0 (
    echo.
    echo Retrying without --only-binary for source distributions...
    "%WHEEL_ENV%\Scripts\pip.exe" download -r backend\requirements.txt -d pip-wheels --python-version 312
)
