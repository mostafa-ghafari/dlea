...
function Portfolios() {
  ...
  const atPortfolioLimit =
    limits.maxPortfolios > 0 && portfolios.length >= limits.maxPortfolios;
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        `به‌روزرسانی پرتفولیو ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
      return;
    }
    setEditing(null);
  ...
  async function toggleArchive(p: Portfolio) {
    ...
    try {
      ...
      toast.success(
        next === "آرشیو" ? `${p.name} آرشیو شد` : `${p.name} از آرشیو خارج شد`,
      );
    } catch (err) {
      toast.error(
        `تغییر وضعیت پرتفولیو ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function remove(p: Portfolio) {
    try {
      ...
      toast.error(
        `حذف پرتفولیو ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        `ساخت پرتفولیو ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
      return;
    }
    setStrategy("");
    ...
    <AppShell
      ...
      actions={
        ...
        >
          ...
          {atPortfolioLimit ? "سقف پرتفولیو پر شد" : "پرتفولیو جدید"}
        </Button>
      }
    >
        ...
        <DialogContent>
          <form onSubmit={submit}>
            <DialogHeader>
              ...
              <DialogDescription>
                یک حساب معاملاتی جدید اضافه کن. بعداً می‌توانی به MT4/MT5 متصل
                کنی.
              </DialogDescription>
            </DialogHeader>
          ...
          <div className="mx-2 h-1.5 flex-1 overflow-hidden rounded-full bg-secondary">
            <div
              className={cn(
                ...
                portfolios.length >= limits.maxPortfolios
                  ? "bg-destructive"
                  : "bg-primary",
              )}
              style={{
                width: `${Math.min(100, (portfolios.length / limits.maxPortfolios) * 100)}%`,
              }}
            />
          </div>
          {atPortfolioLimit && (
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
                <div>
                  <span className="text-muted-foreground">لوریج:</span>{" "}
                  <span className="tabular">{p.leverage}</span>
                </div>
                <div>
        ...
        <DialogContent>
          <form onSubmit={saveEdit}>
            <DialogHeader>
              ...
              <DialogDescription>
                اطلاعات حساب معاملاتی را به‌روزرسانی کن.
              </DialogDescription>
            </DialogHeader>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditForm({ ...editForm, leverage: v })
                  }
                >
                  <SelectTrigger className="bg-secondary/60">
                    <SelectValue />
