...
function AdminPaymentsPage() {
  ...
  const payTotalPages = Math.max(
    1,
    Math.ceil(filteredPayments.length / PAY_PAGE_SIZE),
  );
  const paySafePage = Math.min(payPage, payTotalPages);
  const paginatedPayments = filteredPayments.slice(
    (paySafePage - 1) * PAY_PAGE_SIZE,
    paySafePage * PAY_PAGE_SIZE,
  );
    ...
    <div className="card-surface p-5">
      ...
      {filteredPayments.length > PAY_PAGE_SIZE && (
        <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
          <span>
            نمایش {(paySafePage - 1) * PAY_PAGE_SIZE + 1}–
            {Math.min(paySafePage * PAY_PAGE_SIZE, filteredPayments.length)} از{" "}
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: Math.min(payTotalPages, 7) }, (_, i) => {
              ...
              else if (paySafePage >= payTotalPages - 3)
                pageNum = payTotalPages - 6 + i;
