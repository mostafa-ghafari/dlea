  ...
  head: () => ({
    meta: [
      { title: "ژورنال معاملاتی | Dlea AI" },
      {
        name: "description",
        content:
          "ثبت، ویرایش، گروه‌بندی و فیلتر ژورنال‌های معاملاتی همراه با اسکرین‌شات و ویرایشگر پیشرفته.",
      },
      { property: "og:title", content: "ژورنال معاملاتی" },
      {
        property: "og:description",
        content:
          "ژورنال‌های خود را با ویرایشگر پیشرفته بنویسید، گروه‌بندی و فیلتر کنید.",
      },
    ],
  }),
...
function JournalPage() {
  ...
  const filtered = useMemo(() => {
    return entries.filter((e) => {
      if (
        query &&
        !`${e.title} ${e.tradeId} ${e.symbol} ${e.emotion}`
          .toLowerCase()
      ...
      if (groupFilter !== "all" && (e.groupId ?? "none") !== groupFilter)
        return false;
  ...
  async function toggleFavorite(id: string, favorite: boolean) {
    try {
      ...
      toast.success(
        favorite ? "از علاقه‌مندی‌ها حذف شد" : "به علاقه‌مندی‌ها اضافه شد",
      );
    } catch (err) {
      toast.error(
        `تغییر وضعیت ژورنال ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      if (editingId) {
        ...
        setEntries((list) =>
          list.map((e) => (e.id === editingId ? updated : e)),
        );
      ...
      toast.error(
        `ثبت ژورنال ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
      return;
    }
    setEditorOpen(false);
  ...
  async function createGroup(ev: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        `ساخت گروه ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function renameGroup(id: string) {
    ...
    try {
      const updated = await renameJournalGroup(id, {
      ...
      toast.error(
        `ویرایش گروه ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function deleteGroup(id: string) {
    try {
      ...
      setEntries((list) =>
        list.map((e) => (e.groupId === id ? { ...e, groupId: null } : e)),
      );
    ...
    >
        ...
        <DialogContent>
          <form onSubmit={createGroup}>
            <DialogHeader>
              ...
              <DialogDescription>
                ژورنال‌ها را در گروه‌های دلخواه دسته‌بندی کن.
              </DialogDescription>
            </DialogHeader>
            <div className="mt-4 space-y-3">
              ...
              <div className="space-y-2 pt-2">
                <div className="text-xs text-muted-foreground">
                  گروه‌های موجود — قابل ویرایش و حذف
                </div>
                {groups.length === 0 && (
      ...
      <Dialog open={filterOpen} onOpenChange={setFilterOpen}>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              نتایج را با معیارهای زیر محدود کن.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={planFilter}
                onValueChange={(v) => setPlanFilter(v as "all" | "yes" | "no")}
              >
                <SelectTrigger className="bg-secondary/60">
                  <SelectValue />
                </SelectTrigger>
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <form onSubmit={submit}>
            <DialogHeader>
              <DialogTitle>
                {editingId ? "ویرایش ژورنال" : "ژورنال جدید"}
              </DialogTitle>
              <DialogDescription>
                یادداشت خود درباره یک معامله را با ویرایشگر پیشرفته بنویس.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <Input
                    value={draft.tradeId}
                    onChange={(e) =>
                      setDraft({ ...draft, tradeId: e.target.value })
                    }
                    placeholder="T-1043"
                    className="tabular bg-secondary/60"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={draft.emotion}
                    onValueChange={(v) => setDraft({ ...draft, emotion: v })}
                  >
                    <SelectTrigger className="bg-secondary/60">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setDraft({ ...draft, groupId: v === "none" ? null : v })
                    }
                  >
                    <SelectTrigger className="bg-secondary/60">
                      <SelectValue />
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setDraft({ ...draft, mistakes: e.target.value })
                    }
                    className="bg-secondary/60"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={draft.lesson}
                    onChange={(e) =>
                      setDraft({ ...draft, lesson: e.target.value })
                    }
                    className="bg-secondary/60"
                  />
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="flex items-center justify-between rounded-lg bg-secondary/40 p-3">
                  ...
                  <Switch
                    checked={draft.plan}
                    onCheckedChange={(v) => setDraft({ ...draft, plan: v })}
                  />
                </div>
                <div className="flex items-center justify-between rounded-lg bg-secondary/40 p-3">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) => setDraft({ ...draft, favorite: v })}
                  />
                </div>
              </div>
            </div>
      ...
      {activeFilters && (
        <div className="mb-4 flex flex-wrap items-center gap-2 text-xs">
          ...
          {symbolFilter !== "all" && (
            <Badge variant="outline">نماد: {symbolFilter}</Badge>
          )}
          {emotionFilter !== "all" && (
            <Badge variant="outline">احساس: {emotionFilter}</Badge>
          )}
          {groupFilter !== "all" && (
            <Badge variant="outline">
              گروه:{" "}
              {groupFilter === "none" ? "بدون گروه" : groupName(groupFilter)}
            </Badge>
          )}
          {periodFilter !== "all" && (
            <Badge variant="outline">دوره: {periodFilter}</Badge>
          )}
          {planFilter !== "all" && (
            <Badge variant="outline">
              {planFilter === "yes" ? "طبق پلن" : "خارج از پلن"}
            </Badge>
          )}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  >
                    <Star
                      className={`h-4 w-4 ${j.favorite ? "fill-current" : ""}`}
                    />
                  </button>
                  <button
                    type="button"
