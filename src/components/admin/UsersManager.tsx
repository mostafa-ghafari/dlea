...
export function UsersManager() {
  ...
  const userPayments = (name: string) =>
    userPaymentsList.filter((p) => p.user === name);
  ...
  async function applyPlan() {
    ...
    try {
      ...
      setRows((list) =>
        list.map((u) => (u.id === planTarget.id ? updated : u)),
      );
      toast.success(
        `پلن ${planTarget.name} بدون پرداخت به ${newPlan} تغییر کرد`,
      );
      setPlanTarget(null);
    } catch (err) {
      toast.error(
        `تغییر پلن ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function applyEmail() {
    ...
    try {
      ...
      setRows((list) =>
        list.map((u) => (u.id === emailTarget.id ? updated : u)),
      );
      toast.success("ایمیل کاربر تغییر کرد و در Audit Log ثبت شد");
      setEmailTarget(null);
    } catch (err) {
      toast.error(
        `تغییر ایمیل ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function applyRole() {
    ...
    try {
      ...
      setRows((list) =>
        list.map((u) => (u.id === roleTarget.id ? updated : u)),
      );
      const rl =
      ...
      toast.error(
        `تغییر نقش ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function removeUser() {
    ...
    try {
      ...
      toast.error(
        `حذف کاربر ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  async function toggleStatus(u: AdminUser) {
    ...
    try {
      ...
      toast.error(
        `تغییر وضعیت ناموفق بود: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
    ...
    <div className="card-surface p-5">
              ...
              >
                ...
                <td className="py-3">
                  <Badge
                    variant="outline"
                    className={
                      u.role === "vip" || u.role?.includes("vip")
                        ? "border-accent/40 bg-accent/10 text-accent"
                        : u.role === "admin"
                          ? "border-destructive/40 bg-destructive/10 text-destructive"
                          : u.role === "master" || u.role === "master-vip"
                            ? "border-primary/40 bg-primary/10 text-primary"
                            : u.role === "professional" ||
                                u.role === "professional-vip"
                ...
                <td className="py-3 text-xs tabular text-muted-foreground">
                  {userPayments(u.name).length} مورد
                </td>
                <td className="py-3">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-52">
                      ...
                      >
                        <CreditCard className="ml-2 h-4 w-4" /> تغییر پلن بدون
                        پرداخت
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onSelect={() => {
      ...
      {totalCount > USER_PAGE_SIZE && (
        <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
          <span>
            نمایش {(userSafePage - 1) * USER_PAGE_SIZE + 1}–
            {Math.min(userSafePage * USER_PAGE_SIZE, totalCount)} از{" "}
            {totalCount} کاربر
          </span>
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: Math.min(userTotalPages, 7) }, (_, i) => {
              ...
              else if (userSafePage >= userTotalPages - 3)
                pageNum = userTotalPages - 6 + i;
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              تغییر ایمیل فقط توسط مدیر ممکن است و به‌صورت خودکار در Audit Log
              ثبت می‌شود.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              پلن به‌صورت دستی و بدون نیاز به پرداخت اعمال می‌شود.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4 space-y-2">
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              کاربر «{removeTarget?.name}» و تمام داده‌هایش حذف می‌شود. این عمل
              قابل بازگشت نیست.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-6">
