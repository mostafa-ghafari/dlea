...
export function formatNum(n: number) {
  if (n >= 1_000_000_000)
    return (
      (n / 1_000_000_000).toLocaleString("fa-IR", {
        maximumFractionDigits: 1,
      }) + " میلیارد"
    );
  if (n >= 1_000_000)
    return (
      (n / 1_000_000).toLocaleString("fa-IR", { maximumFractionDigits: 1 }) +
      " میلیون"
    );
  if (n >= 1_000)
    return (
      (n / 1_000).toLocaleString("fa-IR", { maximumFractionDigits: 1 }) +
      " هزار"
    );
...
export function AdminPagination({
  ...
  for (const p of pages) {
    if (typeof p === "number" && prev !== -1 && p - prev > 1)
      withEllipsis.push("...");
    withEllipsis.push(p);
