    ...
    <>
      ...
      >
        ...
        <span className="truncate">
          {isAdmin
            ? "جستجو در بخش‌های مدیریت..."
            : "جستجو در صفحات، معاملات و ژورنال..."}
        </span>
      </button>
  ...
  const results = useMemo(() => {
    ...
    if (isAdmin) {
      ...
      return {
        ...
        journals: [],
        users: allUsers
          .filter((u) => `${u.name} ${u.email}`.toLowerCase().includes(term))
          .slice(0, 6),
      } as const;
    }
    if (!term)
    ...
    return {
      ...
      trades: trades
        .filter((t) =>
          `${t.symbol} ${t.id} ${t.ticket} ${t.strategy}`
            .toLowerCase()
            .includes(term),
        )
        .slice(0, 6),
      journals: journalEntries
        .filter((j) =>
          `${j.title} ${j.symbol ?? ""}`.toLowerCase().includes(term),
        )
        .slice(0, 5),
      users: [],
    } as const;
  ...
  return (
    <DialogContent className="max-w-lg">
      <DialogHeader>
        ...
        <DialogDescription>
          {isAdmin
            ? "نام بخش مدیریت را بنویس."
            : "نام صفحه، نماد معامله، شماره تیکت یا عنوان ژورنال را بنویس."}
        </DialogDescription>
      </DialogHeader>
      <Input
        autoFocus
          ...
          <div>
            ...
            {results.users.map((u) => (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-right text-sm hover:bg-secondary/70"
              >
                <Users className="h-4 w-4 text-primary" /> {u.name}{" "}
