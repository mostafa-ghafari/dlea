...
export function NavList({
  ...
  const regularItems = !roleLoaded
    ? []
    : isAdmin
      ? []
      : nav.filter((item) => !item.admin);
    ...
    <>
      <ul className="space-y-1">
        {regularItems.map((item) => {
          ...
          const locked =
            item.feature !== null && !limits.features.includes(item.feature);
          return (
            <li key={item.to}>
              {locked ? (
                <button
                ...
                >
                  ...
                  {!collapsed && (
                    <Lock className="mr-auto h-3 w-3 text-amber-500" />
                  )}
                </button>
              ) : (
                <Link
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {lockedFeature === "ai-coach" &&
                "مربی هوشمند فقط در پلن Pro و بالاتر فعال است."}
              {lockedFeature === "risk" &&
                "مدیریت ریسک فقط در پلن Pro و بالاتر فعال است."}
              {lockedFeature === "mt-connection" &&
                "اتصال MetaTrader فقط در پلن Pro و بالاتر فعال است."}
              {lockedFeature === "reports" &&
                "گزارش‌های پیشرفته فقط در پلن Pro و بالاتر فعال است."}
              {lockedFeature === "psychology" &&
                "تحلیل روانشناسی فقط در پلن Pro Max فعال است."}
              {!"ai-coach risk mt-connection reports psychology".includes(
                lockedFeature ?? "",
