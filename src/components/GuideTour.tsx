...
export const tours: Record<string, TourStep[]> = {
  ...
  "/app/goals": [
    { title: "اهداف", body: "هدف ماهانه تعریف کن و درصد پیشرفتش را دنبال کن." },
  ],
  "/app/billing": [
    {
      title: "اشتراک",
...
function PortfolioReminderDialog() {
  ...
  const [seen, setSeen, ready] = useLocalState<boolean>(
    PORTFOLIO_GATE_KEY,
    false,
  );
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogTitle className="text-center">
            برای شروع، اول پرتفولیو بساز
          </DialogTitle>
          <DialogDescription className="text-center leading-relaxed">
            تمام بخش‌های پلتفرم (معاملات، ژورنال، اهداف و ...) نیاز به حداقل یک
            پرتفولیو دارند. تا زمانی که پرتفولیو نسازی، امکان استفاده از این
            بخش‌ها وجود ندارد.
          </DialogDescription>
        </DialogHeader>
...
export function GuideTour({
  ...
  const [seen, setSeen, ready] = useLocalState<Record<string, boolean>>(
    "tj:tours-seen:v2",
    {},
  );
  const [open, setOpen] = useState(false);
